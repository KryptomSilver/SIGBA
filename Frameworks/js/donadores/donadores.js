  ...
  //  $(window).on('scroll', function () {
   ...
   // });
    ...
    $('#formdelete').submit(e => {
        ...
        const url = 'procesos/donadores/donadores_delete.php';
        console.log(postData, url);
        $.post(url, postData, (response) => {
            console.log(response);
            if (response == 'DONADOR ELIMINADO') {
                listar();
                $('#delete').modal('hide');
...
// LISTAR PROVEEDORES
var listar = function () {
    var table = $('#donadores').DataTable({
        "destroy": true,
        "ajax": {
            "method": "POST",
            "url": "procesos/donadores/donadores_listar.php"
        },
        "columnDefs": [{
    ...
    edit_data("#donadores tbody", table);
    delete_data("#donadores tbody", table);
    see_data("#donadores tbody", table);
}
...
var see_data = function (tbody, table) {
    ...
    $(tbody).on("click", "a.ver", function () {
        ...
        window.location.href = 'procesos/seeall.php?idpersona=' + idpersona+'&i=3';
    });
}